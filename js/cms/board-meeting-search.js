...
app.factory('requestFactory', ['$http', function($http) {
    ...
    return {
        getCheckboxItems: function(title) {
            return $http.get('/json/board-meeting-checkbox.json',jsonConfig).then(function(result) {
                result.data.pop();
                return result.data;
            },function() {
                $(".angularEvents").remove();
        ...
        getItems: function(title) {
            return $http.get('/json/board-meetings.json',jsonConfig).then(function(result) {
                return result.data;
            },function() {
                $(".angularEvents").remove();
                $(".js__Hide").removeClass('js__Hide');
...
app.controller('searchController', ['$scope', '$sce', 'selectedFilter', 'selectedPressFilter', 'dateRangeFilter', 'requestFactory', '$filter', function($scope, $sce, selectedFilter, selectedPressFilter, dateFilter, requestFactory, $filter) {
    $scope.checked = JSON.parse(sessionStorage.getItem("boardchecked")) || [];
    $scope.query = "";
    $scope.datepicker = {};
    ...
    $scope.changeValue = function(e) {
        ...
        sessionStorage.setItem("boardchecked", JSON.stringify($scope.checked));
        sessionStorage.setItem("boardquery", JSON.stringify($scope.query));
        sessionStorage.setItem("boardstart", $scope.datepicker.startDate);
        sessionStorage.setItem("boardend", $scope.datepicker.endDate);
        $scope.changeFilter();
    };
  ...
  var minDate = Math.min.apply(Math, $scope.items.map(function(o) {
        ...
        $scope.query = JSON.parse(sessionStorage.getItem("boardquery")) || "";
        ...
        if (sessionStorage.getItem("boardend")) {
            $scope.datepicker.endDate = $scope.datepicker.end = new Date(sessionStorage.getItem("boardend"));
        } else {
            $scope.datepicker.endDate = maxDate;
        }
         if (sessionStorage.getItem("boardstart")) {
        $scope.datepicker.startDate = $scope.datepicker.start = new Date(sessionStorage.getItem("boardstart"));
    } else {
