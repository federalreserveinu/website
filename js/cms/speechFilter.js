...
app.factory('requestFactory', ['$http', function($http) {
    ...
    return {
        ...
        getItems: function(title) {
            return $http.get('/json/ne-speeches.json',jsonConfig).then(function(result) {
                return result.data;
            },function() {
                $(".angularEvents").remove();
                $(".js__Hide").removeClass('js__Hide');
...
app.controller('searchController', ['$scope', '$sce', 'selectedFilter', 'selectedPressFilter', 'dateRangeFilter', 'requestFactory', '$filter', function($scope, $sce, selectedFilter, selectedPressFilter, dateFilter, requestFactory, $filter) {
    $scope.checked = JSON.parse(sessionStorage.getItem("speechchecked")) || [];
    $scope.query = "";
    $scope.datepicker = {};
    ...
    $scope.changeValue = function(e) {
        ...
        sessionStorage.setItem("speechchecked", JSON.stringify($scope.checked));
        sessionStorage.setItem("speechquery", JSON.stringify($scope.query));
        sessionStorage.setItem("speechstart", $scope.datepicker.startDate);
        sessionStorage.setItem("speechend", $scope.datepicker.endDate);
        $scope.changeFilter();
    };
  ...
  var minDate = Math.min.apply(Math, $scope.items.map(function(o) {
        ...
        $scope.query = JSON.parse(sessionStorage.getItem("speechquery")) || "";
        ...
        if (sessionStorage.getItem("speechend")) {
            $scope.datepicker.endDate = $scope.datepicker.end = new Date(sessionStorage.getItem("speechend"));
        } else {
            $scope.datepicker.endDate = maxDate;
        }
         if (sessionStorage.getItem("speechstart")) {
        $scope.datepicker.startDate = $scope.datepicker.start = new Date(sessionStorage.getItem("speechstart"));
    } else {
