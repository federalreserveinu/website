...
app.factory('requestFactory', ['$http', function($http) {
    ...
    return {
        getCheckboxItems: function(title) {
            return $http.get('/json/nespeakers.json',jsonConfig).then(function(result) {
                result.data.pop();
                return result.data;
            },function() {
                $(".angularEvents").remove();
        ...
        getItems: function(title) {
            return $http.get('/json/ne-testimony.json',jsonConfig).then(function(result) {
                return result.data;
            },function() {
                $(".angularEvents").remove();
                $(".js__Hide").removeClass('js__Hide');
...
app.controller('searchController', ['$scope', '$sce', 'selectedFilter', 'selectedPressFilter', 'dateRangeFilter', 'requestFactory', '$filter', function($scope, $sce, selectedFilter, selectedPressFilter, dateFilter, requestFactory, $filter) {
    $scope.checked = JSON.parse(sessionStorage.getItem("testimonychecked")) || [];
    $scope.query = "";
    $scope.datepicker = {};
    ...
    $scope.changeValue = function(e) {
        ...
        sessionStorage.setItem("testimonychecked", JSON.stringify($scope.checked));
        sessionStorage.setItem("testimonyquery", JSON.stringify($scope.query));
        sessionStorage.setItem("testimonystart", $scope.datepicker.startDate);
        sessionStorage.setItem("testimonyend", $scope.datepicker.endDate);
        $scope.changeFilter();
    };
  ...
  var minDate = Math.min.apply(Math, $scope.items.map(function(o) {
        ...
        $scope.query = JSON.parse(sessionStorage.getItem("testimonyquery")) || "";
        ...
        if (sessionStorage.getItem("testimonyend")) {
            $scope.datepicker.endDate = $scope.datepicker.end = new Date(sessionStorage.getItem("testimonyend"));
        } else {
            $scope.datepicker.endDate = maxDate;
        }
         if (sessionStorage.getItem("testimonystart")) {
        $scope.datepicker.startDate = $scope.datepicker.start = new Date(sessionStorage.getItem("testimonystart"));
    } else {
